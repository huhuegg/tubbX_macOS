...
final class AccessibilityElement {
    ...
    func bringToFront() {
        if let isMainWindow = self.rawValueForAttribute(NSAccessibilityMainAttribute) as? Bool, isMainWindow
        {
            return
        }
    ...
    fileprivate func valueForAttribute(_ attribute: String) -> Self? {
        if let rawValue = self.rawValueForAttribute(attribute), CFGetTypeID(rawValue) == AXUIElementGetTypeID()
        {
    ...
    fileprivate func valueForAttribute<T>(_ attribute: String) -> T? {
        if let rawValue = self.rawValueForAttribute(attribute), CFGetTypeID(rawValue) == AXValueGetTypeID()
        {
